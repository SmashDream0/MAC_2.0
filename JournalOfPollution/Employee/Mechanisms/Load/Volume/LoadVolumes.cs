    ...
    public class LoadVolumes : MyTools.C_A_BaseLoad_Excel
    {
        ...
        private void Compare()
        {
            ...
            foreach (var one in temp.GroupBy(x => x.INN))
            {
                if (one.Count() > 1)
                {
                    var fils = _file.Where(x => x.INN == one.Key);
                    if (fils != null)
                    {
                        foreach (var fil in fils)
                        { fil.SetClient(one.Select(x => x.sw).ToArray()); }
                        _files.AddRange(fils);
                    }
                }
                else
                {
                    var fil = _file.FirstOrDefault(x => x.INN == one.Key);
                    if (fil != null)
                    { fil.SetClient(one.First().sw); }
                    else
                    { fil = new Item(one.First().sw); }
                    _files.Add(fil);
                }
            }
        }
        ...
        private void ComparisonVolume()
        {
            ...
            foreach (var one in _volumes)
            {
                var fil = _files.FirstOrDefault(x => x.sample != null && x.sample.ID == one.SampleID);
                if (fil != null)
                {
            ...
            foreach (var one in _files.Where(x => x.objecte != null && x.Volume > 0).ToArray())
            {
