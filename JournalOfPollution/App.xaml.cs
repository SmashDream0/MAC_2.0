...
namespace MAC_2
{
    ...
    public partial class App : Application
    {
        static App()
        {
            ...
            MN(Environment.GetCommandLineArgs());//аргументы командной строки
        }
        ...
        public static class AutoUpdate
        {
            ...
            public static string Get7zdllName()
            {
                ...
                switch (si.wProcessorArchitecture)
                {
                    ...
                    case SYSTEM_INFO.PROCESSOR_ARCHITECTURE.IA64:
                        if (!System.IO.File.Exists("7z64_cp.dll"))
                        { return null; }
                        else
                        { return "7z64_cp.dll"; }
                    case SYSTEM_INFO.PROCESSOR_ARCHITECTURE.INTEL:
                        if (!System.IO.File.Exists("7z32_cp.dll"))
                        { return null; }
                        else
                        { return "7z32_cp.dll"; }
            ...
            public class settings_class
            {
                ...
                string Path { get { return StartupPath + "\\" + SettingsFileName; } }
                ...
                static string DeCryptItString(System.IO.Stream str, int location, int Length) //Расшифровывает текст
                {
                    var bytes = new byte[Length];
                    str.Position = location;
                    str.Read(bytes, 0, Length);

                    return System.Text.Encoding.Default.GetString(Crypting(bytes, location, -1)).Replace("\0", string.Empty);
                }
                ...
                static void CryptIt(string Volume, System.IO.Stream str, int location, int length)
                {
                    var bytes = new byte[length];

                    if (Volume.Length > length)
                    { System.Text.Encoding.Default.GetBytes(Volume, 0, length, bytes, 0); }
                    else
                    { System.Text.Encoding.Default.GetBytes(Volume, 0, Volume.Length, bytes, 0); }
                ...
                public System.Security.Authentication.SslProtocols MailSecure
                {
                    get
                    {
                        ...
                        switch ((System.Security.Authentication.SslProtocols)b)
                        {
                            ...
                            case System.Security.Authentication.SslProtocols.Tls:
                                return (System.Security.Authentication.SslProtocols)b;
            ...
            public static void UpdateAutoUpdate(string FileName, string Command)
            {
                ...
                switch (Settings.UpdateType)
                {
                    case settings_class.UpdateType_enum.Mail:
                        if (!System.IO.File.Exists("ImapX_cp.dll"))
                        {
                        ...
                        }
                        {
                            ...
                            new AutoTable.Forms.Progress_Form(s).ShowDialog();
                            FilePath = s.FilePath;
                            date = s.Date;
                        }
                    ...
                    case settings_class.UpdateType_enum.Path:
                        ...
                        if (Settings.FileUpdateLocation.Length == 0 || !System.IO.File.Exists(Settings.FileUpdateLocation))
                        { FilePath = null; }
                        else
                        {
                ...
                if (FilePath != null && System.IO.File.Exists(FilePath))
                {
...

                    {
                        ...
                        if ((bool)new AutoTable.Forms.Progress_Form(new Extract_class(FilePath, Password)).ShowDialog())
                        {
            ...
            {
                var SW = (new System.IO.StreamWriter(StartupPath + "\\" + Application.Current.MainWindow.GetType().Assembly.FullName + "_Log.txt", false));
                SW.WriteLine(DateTime.Now.ToString() + " - " + Message);
                SW.Close();
            }

            class Extract_class : AutoTable.Forms.Progress_Form.AObject
            {
                ...
                {
                    SevenZip.SevenZipExtractor extr;

                    if (ArchPass.Length > 0)
                    { extr = new SevenZip.SevenZipExtractor(ArchPath, ArchPass); }
                    else
                    { extr = new SevenZip.SevenZipExtractor(ArchPath); }

                    extr.Extracting += (pdn, pdt) => Action("Распаковка", 100, pdt.PercentDone);
                    extr.ExtractArchive(StartupPath);
                    extr.Dispose();
            ...
            class FindUpdate_class : AutoTable.Forms.Progress_Form.AObject
            {
                public FindUpdate_class(System.Text.StringBuilder lines, settings_class Settings)
                    : base(true)
                {
                    this.lines = lines;
                ...
                protected override bool Do()
                {
                    ...
                    var SearchedSubject = System.IO.Path.GetFileNameWithoutExtension(Settings.FileName);
                    ...
                    if (LastDateIndex > -1)
                    {
                        ...
                        if (messages[LastDateIndex].Attachments.Length == 1)
                        {
                            ...
                            FilePath = System.IO.Path.GetTempPath() + messages[LastDateIndex].Attachments[0].FileName;
